...
class Agent:
    ...
    def __init__(self, env, gamma=0.9, alpha=0.1, epsilon=1.0, epsilon_decay=0.01, epsilon_min=1, episodes=1000):
        self.env = env
        self.gamma = gamma
        self.alpha = alpha
        self.epsilon = epsilon
        self.epsilon_decay = epsilon_decay
        self.epsilon_min = epsilon_min  # Minimum epsilon to avoid zero exploration
    ...
    def train(self, verbose=False):
        ...
        Parameters:
            verbose (bool): Whether to print episode progress and stats.
        ...
        for episode in range(self.episodes):
            ...
            if verbose:
                print(f"Episode {episode + 1}/{self.episodes} - Total Reward: {total_reward}, Epsilon: {self.epsilon}")
...
class QLearningAgent(Agent):
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        td_target = reward + self.gamma * np.max(self.q_table[next_state_index]) * (not done)
