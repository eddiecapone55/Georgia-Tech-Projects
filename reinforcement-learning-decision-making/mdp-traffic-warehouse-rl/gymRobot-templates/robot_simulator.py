...
class RobotSim:
    ...
    def move_workers(self):
        ...
        """
        # for each worker on the array, try to move them randomly by no more than a step away from their original position
        for worker_index in range(len(self.warehouse_workers)):
            ...
            if (0 <= next_position[0] < self.n_rows) and (0 <= next_position[1] < self.n_cols):
                ...
                if np.any(np.all(next_position == self.reachable_locations, axis=1)) and np.sum(np.abs(next_position-original_position)) < 2:
