...
class TrafficEnv(gym.Env):
    ...
    def __init__(self, max_cars_dir=20, max_cars_total=30, lambda_ns=2, lambda_ew=3, cars_leaving=5, rewards=None, max_steps=1000):
        """
    ...
    def get_rewards(self, ns, ew, light):
        ...
        """
        # Reward for clearing cars when the light is green
        if light == 1:  # NS is green, EW is red
            reward_ns = -ns * 0.5  # Penalize more if there are more cars waiting in NS (since NS should be clearing)
            reward_ew = -ew * 2    # Penalize more heavily for EW cars waiting (since they can't move)
        else:  # NS is red, EW is green
            reward_ns = -ns * 2    # Penalize more heavily for NS cars waiting (since they can't move)
            reward_ew = -ew * 0.5  # Penalize less for EW cars since they should be clearing

        # Combine rewards
        total_reward = reward_ns + reward_ew
    
        # Optionally add a penalty if total cars exceed a certain limit
        if ns + ew >= self.max_cars_total:
            total_reward -= 10  # Add an extra penalty for high congestion
    ...
    def is_terminal(self, ns, ew):
        ...
        """
        # Terminal if the total number of cars exceeds the allowed maximum
    ...
    def step(self, action):
        ...
        # Get the updated world state
        ns, ew, light, prob = self.sim.get_world_state()
    
        # Compute the reward
        reward = self.get_rewards(ns, ew, light)
    
        # Check if the episode is done (terminal state or truncated)
        done = self.is_terminal(ns, ew)
        truncated = self.is_truncated()
    
        # Update the current state and step count
        self.s = (ns, ew, light)
        self.current_step += 1
    
        # Return the observation, reward, done flag, and additional info
        return np.array(self.s), reward, done, truncated, {"prob": prob}
