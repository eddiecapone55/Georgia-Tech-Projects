...
def load_mnist_trainval():
    ...
    :return:
        train_data: A list of lists containing the training data
        train_label: A list containing the labels of training data
        val_data: A list of lists containing the validation data
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    :param data: A list of lists containing the data where each inner list contains 28x28
    ...
    # Shuffle data and labels together if shuffle=True
    if shuffle:
    ...
    batched_data = [np.array(data[i:i + batch_size]) for i in range(0, len(data), batch_size)]
    batched_label = [np.array(label[i:i + batch_size]) for i in range(0, len(label), batch_size)]
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    """
    Plot learning curves with matplotlib. Make sure training loss and validation loss are plotted in the same figure and
    training accuracy and validation accuracy are plotted in the same figure too.
